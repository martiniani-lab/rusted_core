...
use pyo3::prelude::{pymodule, pyfunction, PyModule, PyResult, Python, Bound};
...
fn compute_sann_boops_2d<'py>(
    ...
    let array = unsafe { points.as_array() };
    let boop_order_array = unsafe { orders.as_array() };
    let boops = voronoi::compute_sann_boops_2d(
        &array, &boop_order_array, box_size, box_size, periodic);
    PyArray::from_array(py, &boops)
}

#[pyfunction]
fn compute_sann_boops_3d<'py>(
    py: Python<'py>,
    points: &Bound<'py, PyArrayDyn<f64>>,
    orders: &Bound<'py, PyArrayDyn<isize>>,
    box_size: f64,
    ...
    let array = unsafe { points.as_array() };
    let boop_order_array = unsafe { orders.as_array() };
    let boops = voronoi::compute_sann_boops_3d(
        &array, &boop_order_array, box_size, box_size, box_size, periodic);
    PyArray::from_array(py, &boops)
}
...
fn rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(compute_vector_rdf2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_vector_rdf3d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_vector_rdf2sphere, m)?)?;
    m.add_function(wrap_pyfunction!(compute_bounded_vector_rdf2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_nnbounded_vector_rdf2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_pnn_vector_rdf2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_pnn_rdf, m)?)?;
    m.add_function(wrap_pyfunction!(compute_bounded_vector_rdf3d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_nnbounded_vector_rdf3d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_pnn_vector_rdf3d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_vector_gyromorphic_corr_2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_radial_correlations_2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_radial_gyromorphic_corr_2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_radial_correlations_3d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_radial_correlations_2sphere, m)?)?;
    m.add_function(wrap_pyfunction!(compute_2d_boops, m)?)?;
    m.add_function(wrap_pyfunction!(compute_2sphere_boops, m)?)?;
    m.add_function(wrap_pyfunction!(compute_metric_boops_2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_metric_boops_3d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_sann_boops_2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_sann_boops_3d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_sann_quantities_2d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_sann_quantities_3d, m)?)?;
    m.add_function(wrap_pyfunction!(compute_2sphere_all_voronoi_quantities, m)?)?;
    m.add_function(wrap_pyfunction!(compute_2d_all_voronoi_quantities, m)?)?;
    m.add_function(wrap_pyfunction!(compute_2d_voronoi_areas, m)?)?;
    m.add_function(wrap_pyfunction!(compute_2d_voronoi_neighbour_numbers, m)?)?;
    m.add_function(wrap_pyfunction!(compute_2d_voronoi_nn_distances, m)?)?;
    m.add_function(wrap_pyfunction!(voronoi_furthest_sites, m)?)?;
    m.add_function(wrap_pyfunction!(voronoi_tessellation_2d, m)?)?;
    m.add_function(wrap_pyfunction!(voronoi_tessellation_2sphere, m)?)?;
    m.add_function(wrap_pyfunction!(compute_pnn_distances, m)?)?;
    m.add_function(wrap_pyfunction!(compute_pnn_mean_nnbound_distances, m)?)?;
    m.add_function(wrap_pyfunction!(cluster_by_distance, m)?)?;
    m.add_function(wrap_pyfunction!(point_variances, m)?)?;
    m.add_function(wrap_pyfunction!(point_skewnessess, m)?)?;
    m.add_function(wrap_pyfunction!(point_variances_2sphere, m)?)?;
    m.add_function(wrap_pyfunction!(metric_neighbors, m)?)?;
    m.add_function(wrap_pyfunction!(monodisperse_metric_neighbors, m)?)?;
    m.add_function(wrap_pyfunction!(metric_neighbors_2sphere, m)?)?;
    m.add_function(wrap_pyfunction!(monodisperse_metric_neighbors_2sphere, m)?)?;
    Ok(())
}
